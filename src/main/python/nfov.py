...
class NFOV():
    def __init__(self, height=400, width=800):
        self.FOV = [float(sys.argv[2]), float(sys.argv[2])]
        self.PI = pi
        self.PI_2 = pi * 0.5
        self.PI2 = pi * 2.0
        self.height = height
    ...
    def _bilinear_interpolation(self, screen_coord, projectionTitle, imageTitle):
        ...
        # plt.imshow(nfov)
        # plt.savefig("my-image.jpg")

        img = Image.fromarray(nfov, 'RGB')
        img.save(imageTitle + "_" + projectionTitle + '.png')
        return nfov

    def toNFOV(self, frame, center_point, projectionTitle, imageTitle):
        self.frame = frame
        ...
        return self._bilinear_interpolation(spericalCoord, projectionTitle, imageTitle)

    def processImage(self, img, imageTitle):
        center_point = np.array([0, 0.5])  # camera center point (valid range [0,1])
        nfov.toNFOV(img, center_point, "BACK", imageTitle)

        center_point = np.array([0.25, 0.5])  # camera center point (valid range [0,1])
        nfov.toNFOV(img, center_point, "LEFT", imageTitle)

        center_point = np.array([0.5, 0.5])  # camera center point (valid range [0,1])
        nfov.toNFOV(img, center_point, "FRONT", imageTitle)

        center_point = np.array([0.75, 0.5])  # camera center point (valid range [0,1])
        nfov.toNFOV(img, center_point, "RIGHT", imageTitle)
...
if __name__ == '__main__':
    ...
    for image in glob.glob(sys.argv[1] + "/*.jpg"):
        nfov.processImage(im.imread(image), image.replace(".jpg", ""))
    end_time = time.time()
    ...
    #     nfov = NFOV(x, x * 2)
    #     processes.append(multiprocessing.Process(target = nfov.processImage(im.imread(image), image.replace(".jpg", ""))))
    #
    # [process.start() for process in processes]
    # end_time = time.time()
    # print("Multiprocess time =", end_time - start_time)
